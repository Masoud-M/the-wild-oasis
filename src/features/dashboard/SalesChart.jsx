...
function SalesChart({ bookings, numDays }) {
  ...
  // calculating how many days of data we need to show in our chart based on the filter(7,30,90 days) and using eachDayInterval function making those dates. it needs a start and an end day. the start would be (today - the filter) and the end day will be Today
  ...
  const data = allDates.map((date) => {
    return {
      // format function is provided by date-fns and it will receive one date and a string to return a formatted date
      label: format(date, "MMM dd"),

      // filtering the bookings to get only those bookings that are in the current date in this loop
      // isSameDay in provided by date-fns and it will compare two dates. it will only accept dates in javascript date object type, that's why we used "new Date" on "booking.created_at" to change it's type from string to javascript date object
      // for example while looping through allDates, we are on "Nov 22", we check how many bookings were in that day "date, newDate(booking.created_at)" and sum their totalPrice or extraPrice
      totalSales: bookings
